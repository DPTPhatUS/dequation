...
def train(args):
    ...
    model = Tex2Eng(model_name="google-t5/t5-small", tokenizer=tokenizer).to(
        args.device
    )
    if args.resume_epoch:
        path = os.path.join(
            args.checkpoint_dir, f"Tex2Eng_epoch_{args.resume_epoch}.pth"
        )
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        for batch_idx, batch in tqdm(
            ...
            inputs = tokenizer(
                batch["equation"], padding=True, truncation=True, return_tensors="pt"
            )
            targets = tokenizer(
                batch["spoken_English"],
                padding=True,
                truncation=True,
                return_tensors="pt",
            )
        ...
        checkpoint_path = os.path.join(
            args.checkpoint_dir, f"Tex2Eng_epoch_{epoch+1}.pth"
        )
        checkpoint_dict = {
